...
void Application::obj_to_terrain_map(){
    ...
    for(auto iter=terrain.mesh.positions.begin(); iter!=terrain.mesh.positions.end(); iter+=3){
        int l_x = (int)*iter;
        int l_z = (int)*(iter+2);
        if(l_x >= - TWIDTH/2 && l_x < (TWIDTH/2-1) && l_z >= - THEIGHT/2 && l_z < (THEIGHT/2-1)){
            // cout<<l_x<<' '<<l_z<<endl;
    ...
    float max = -10;
    // for(int i=0; i<THEIGHT*TWIDTH; i++){
    //     cout<<"density"<<terrain_map_density[i]<<endl;
    // }
    for(int i=0; i<THEIGHT*TWIDTH; i++){
        ...
        if(abs(terrain_map[i])>100){
            cout<<"big h occured "<<i/TWIDTH-THEIGHT/2<<' '<<i%TWIDTH-TWIDTH/2<<'\t'<<terrain_map[i]<<'\t'<<prv<<'\t'<<terrain_map_density[i]<<endl;
        }
    ...
    for(int i=0; i<TWIDTH; i++){
        for(int j=0; j<THEIGHT; j++){
            int h_val = (int) ((terrain_map[i*TWIDTH + j] - min)*range_k);
