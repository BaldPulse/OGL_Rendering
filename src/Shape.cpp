...
void Shape::createShape(tinyobj::shape_t & shape)
{
	...
	mtlBuf = shape.mesh.material_ids;
}
...
void Shape::init()
{
	...
	CHECKED_GL_CALL(glGenBuffers(1, &mtlBufID));
	CHECKED_GL_CALL(glBindBuffer(GL_ARRAY_BUFFER, mtlBufID));
	CHECKED_GL_CALL(glBufferData(GL_ARRAY_BUFFER, mtlBuf.size()*sizeof(int), &mtlBuf[0], GL_STATIC_DRAW));
